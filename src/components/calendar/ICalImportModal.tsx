...
export const ICalImportModal: React.FC<ICalImportModalProps> = ({
  ...
  const handleUrlImport = async () => {
    ...
    try {
      ...
      setValidationError(
        "L'importazione da URL richiede un proxy server. Per ora, scarica il file .ics e caricalo manualmente."
      );
    } catch (error) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${successCount} prenotazioni importate con successo${
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Importa prenotazioni da un file iCal (.ics) o da un URL di
            calendario.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              isLoading || previewEvents.length === 0 || !selectedApartment
            }
          >
            {isLoading && <Upload className="w-4 h-4 mr-2 animate-spin" />}
            Importa{" "}
            {previewEvents.length > 0 ? `(${previewEvents.length})` : ""}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
