...
import { Plus, Edit, Trash2, Users, Euro, ArrowLeft } from "lucide-react";
...
const ApartmentManagement: React.FC = () => {
  ...
  // Funzione per verificare se una combinazione icona+colore è già utilizzata
  const isIconColorCombinationUsed = (
    icon: string,
    color: string,
    excludeId?: string
  ): boolean => {
    return apartments.some(
      (apt) => apt.icon === icon && apt.color === color && apt.id !== excludeId
    );
  };

  // Filtra le icone disponibili per evitare duplicati
  const getAvailableIcons = (currentColor: string, excludeId?: string) => {
    return availableIcons.filter(
      (icon) => !isIconColorCombinationUsed(icon, currentColor, excludeId)
    );
  };

  // Filtra i colori disponibili per evitare duplicati
  const getAvailableColors = (currentIcon: string, excludeId?: string) => {
    return availableColors.filter(
      (color) => !isIconColorCombinationUsed(currentIcon, color, excludeId)
    );
  };
  ...
  const handleEdit = (apartment: Apartment) => {
    ...
    setFormData({
      ...
      color: apartment.color || "#3DA9A9",
      icon: apartment.icon || "Home",
    });
  ...
  const handleDelete = async (apartment: Apartment) => {
    if (
      !confirm(
        `Sei sicuro di voler eliminare l'appartamento "${apartment.name}"? Questa azione eliminerà anche tutte le prenotazioni associate.`
      )
    ) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      {/* Mobile Header */}
      <MobileHeader
        pageTitle="Gestione Alloggi"
        onNewApartmentClick={() => setIsCreateDialogOpen(true)}
      ...
      {/* Desktop Header con statistiche */}
      <div className="hidden md:flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Gestione Alloggi</h2>
          <p className="text-muted-foreground">
            {stats.active} attivi di {stats.total} totali • Capacità:{" "}
            {stats.totalCapacity} ospiti
          </p>
        </div>
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
              ...
              <span className="text-sm text-muted-foreground">
                {formData.name || "Nome alloggio"} - {formData.icon} (
                {formData.color})
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <SelectField
                  ...
                  options={getAvailableIcons(formData.color || "#3DA9A9").map(
                    (iconName) => ({
                      value: iconName,
                      label: iconName,
                      icon: (
              ...
              <div>
                <SelectField
                  id="color"
                  label="Colore"
                  value={formData.color || "#3DA9A9"}
                  onChange={(value) =>
                    setFormData({ ...formData, color: value })
                  }
                  options={getAvailableColors(formData.icon || "Home").map(
                    (color) => ({
                      value: color,
                      label: color,
                      icon: (
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {apartments.map((apartment) => (
          <Card
            key={apartment.id}
            className={!apartment.isActive ? "opacity-60" : ""}
          >
