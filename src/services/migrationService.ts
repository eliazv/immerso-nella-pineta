...
import { Booking, CalendarType, APARTMENT_MAPPING } from "@/types/calendar";
...
// Mapping dei fogli per ogni calendario
const SHEET_MAPPING: Record<CalendarType, string> = {
  principale: "Affitti3",
  secondario: "Affitti4", 
  terziario: "Affitti8",
  all: "" // Non utilizzato per la migrazione
};

/**
 * Migra le prenotazioni da Google Sheets per un singolo calendario
 */
export const migrateBookingsFromSheet = async (calendarType: CalendarType): Promise<number> => {
  if (calendarType === "all") {
    throw new Error("Impossibile migrare per il tipo 'all'. Usa i singoli calendari.");
  }
  ...
  try {
    ...
    const validBookings: Booking[] = data
      ...
      .map((row: Record<string, string>, index: number) => ({
        ...
        Totale: row["Totale"] || "0",
        Note: row["Note"] || "",
        apartment: APARTMENT_MAPPING[calendarType],
        rowIndex: index + 2 // +2 perché la prima riga è l'header e gli indici partono da 1
      }));

    console.log(`Trovate ${validBookings.length} prenotazioni valide per ${calendarType}`);
    ...
    for (const booking of validBookings) {
      try {
        ...
        const existingBookings = localStorageService.getBookingsByApartment(booking.apartment!);
        const exists = existingBookings.some(existing => 
        ...
        if (!exists) {
          ...
          console.log(`Prenotazione già esistente: ${booking.Nome} (${booking.CheckIn} - ${booking.CheckOut})`);
        }
      } catch (error) {
        console.error(`Errore nella migrazione della prenotazione ${booking.Nome}:`, error);
      }
    }
...
export const clearAllBookings = (): void => {
  ...
  console.log("Tutte le prenotazioni sono state eliminate. Backup salvato in 'migration_backup'.");
};
...
export const getMigrationStats = () => {
  ...
  apartments.forEach(apartment => {
    const apartmentBookings = localStorageService.getBookingsByApartment(apartment.id);
...
export const validateMigratedData = (): {
  ...
  bookings.forEach((booking, index) => {
    ...
    if (!booking.CheckIn || !booking.CheckOut) {
      errors.push(`Prenotazione ${index + 1}: Date check-in/check-out mancanti`);
    }
    
    if (!booking.apartment) {
      warnings.push(`Prenotazione ${booking.Nome}: Appartamento non specificato`);
    } else {
      const apartment = localStorageService.getApartmentById(booking.apartment);
      if (!apartment) {
        errors.push(`Prenotazione ${booking.Nome}: Appartamento ${booking.apartment} non trovato`);
      }
    }
  });
