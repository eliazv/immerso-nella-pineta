 ...
 */
export const getApartmentByCalendarType = (calendarType: CalendarType): Apartment | null => {
...
export const createApartment = (data: CreateApartmentData): Apartment => {
  ...
  const nameExists = existingApartments.some(apt => 
    apt.name.toLowerCase() === data.name.toLowerCase()
  );
 ...
 */
export const updateApartment = (id: string, data: UpdateApartmentData): boolean => {
...
export const deleteApartment = (id: string): boolean => {
  ...
  if (bookings.length > 0) {
    ...
    console.warn(`Eliminazione appartamento ${id}: verranno eliminate anche ${bookings.length} prenotazioni associate`);
  }
...
export const getApartmentStats = () => {
  ...
  return {
    ...
    totalCapacity: activeApartments.reduce((sum, apt) => sum + apt.maxGuests, 0),
    averagePrice: activeApartments.length > 0 
      ? activeApartments.reduce((sum, apt) => sum + (apt.basePrice || 0), 0) / activeApartments.length 
      : 0
  };
};
...
export const searchApartments = (query: string): Apartment[] => {
  ...
  return localStorageService.getApartments().filter(apt => 
    apt.name.toLowerCase().includes(searchTerm) ||
    (apt.description && apt.description.toLowerCase().includes(searchTerm)) ||
 ...
 */
export const validateApartmentData = (data: CreateApartmentData | UpdateApartmentData): string[] => {
  const errors: string[] = [];
