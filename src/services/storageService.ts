...
export class StorageService {
  ...
  static async sendViaEmail(
    ...
    try {
      ...
      const { AlloggiatiService } = await import('./alloggiatiService');
      const content = AlloggiatiService.generateSchedina(formData);
      const fileName = AlloggiatiService.generateFileName(formData.idAppartamento);
...
Sistema Alloggiati Web
  ...
  static cleanupExpiredData(): void {
    const keys = Object.keys(localStorage).filter(key => key.startsWith('alloggiati_'));
